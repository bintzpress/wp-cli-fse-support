...
class PackageCore {
	...
	public function list($post_type) {
		global $wpdb;
		$rows = $wpdb->get_results("select ID, post_name as name, post_type as type, post_modified as modified, post_status as status from wp_posts where post_type = '$post_type' and (post_status = 'publish' or post_status = 'draft') order by name, modified desc");
		WP_CLI\Utils\format_items('table', $rows, array('ID', 'name', 'type', 'modified', 'status'));   
	}
